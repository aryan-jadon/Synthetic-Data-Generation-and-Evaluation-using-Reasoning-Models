...
def union_ranges(ranges):
    if not ranges:  # Add this check to avoid the error
        return []
...
class BaseEvaluation:
    ...
    def _full_precision_score(self, chunk_metadatas):
        ...
        for index, row in self.questions_df.iterrows():
            ...
            denominator_sets = union_ranges(denominator_chunks_sets + unused_highlights) if (
                        denominator_chunks_sets or unused_highlights) else []
            ...
            total_reference_range = sum_of_ranges([(x['start_index'], x['end_index']) for x in references])

            # Avoid division by zero
            if total_reference_range == 0:
                recall_score = 1  # Assign maximum recall if there are no reference highlights
            else:
                recall_score = 1 - (sum_of_ranges(unused_highlights) / total_reference_range)
    ...
    def _scores_from_dataset_and_retrievals(self, question_metadatas, highlighted_chunks_count):
        ...
        for (index, row), highlighted_chunk_count, metadatas in zip(self.questions_df.iterrows(), highlighted_chunks_count, question_metadatas):
            ...
            if recall_denominator == 0:
                recall_score = 1  # Assign perfect recall if there’s nothing to recall
            else:
